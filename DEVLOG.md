...
**Branch durumu:** v2.0 (PR #7), pratik modlarındaki eski cevap sızıntısı bugfix'i (PR #8) ve DTZ Sınav Modu UX düzeltmeleri — sticky sayaç + şık karıştırma (PR #9) `main`'e merge edildi (`main` @ `4cb66bc`) — bekleyen PR veya push yok, yeni sohbet `main`'in en güncel hali üzerinden devam edebilir.
